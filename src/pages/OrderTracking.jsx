  ...
  const renderStep = (status, label, icon) => {
    ...
    return (
      <div className={`flex flex-col items-center flex-1 ${isCompleted ? 'text-teal-600' : 'text-gray-300'}`}>
        <div className={`w-10 h-10 rounded-full flex items-center justify-center border-2 ${isCompleted ? 'bg-teal-50 border-teal-600' : 'bg-white border-gray-200'}`}>{icon}</div>
      ...
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl overflow-hidden">
        ...
        <div className="p-6">
          <form onSubmit={handleTrack} className="relative">
            <input type="text" placeholder="e.g. INV-2025001" value={invoiceId} onChange={(e) => setInvoiceId(e.target.value)} className="w-full pl-10 pr-4 py-3 border-2 rounded-xl" />
            <Search className="absolute left-3 top-3.5 text-gray-400 w-5 h-5" />
            <button type="submit" disabled={loading} className="absolute right-2 top-2 bg-teal-600 text-white px-4 py-1.5 rounded-lg text-sm font-bold">{loading ? '...' : 'Track'}</button>
          </form>
        ...
        {order && (
          <div className="border-t border-gray-100">
            ...
            <div className="bg-gray-50 p-6 space-y-3">
              <div className="flex justify-between"><span className="text-gray-500 text-sm">Invoice:</span><span className="font-bold text-gray-800">{order.invoiceId}</span></div>
              <div className="flex justify-between"><span className="text-gray-500 text-sm">Amount:</span><span className="font-bold text-teal-700">৳{order.totalAmount}</span></div>
              ...
              {order.trackingCode && (
                <div className="mt-4 p-3 bg-white border border-teal-100 rounded-lg">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    {/* LINK ADDED HERE */}
                    <a href={`https://steadfast.com.bd/t/${order.trackingCode}`} target="_blank" rel="noreferrer" className="font-mono text-xs bg-teal-50 text-teal-700 px-2 py-1 rounded border border-teal-200 hover:bg-teal-100 flex items-center gap-1">
                      {order.trackingCode} ↗
                    </a>
                  </div>
                </div>
